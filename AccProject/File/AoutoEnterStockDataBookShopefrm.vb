...
Public Class AoutoEnterStockDataBookShopefrm
    ...
    Private Sub SanfQfrm_Load(ByVal sender As Object, ByVal e As EventArgs) Handles MyBase.Load
        ...
        StockBookShope.load()
        T_textbox5.Focus()
        proj.LoadAoutoEnter_StockDataBookShop()
        ...
        proj.StockData_BookShope.StockDataBookShope_Fm = {proj.StockData_BookShope.Code, proj.StockData_BookShope.sanf, proj.StockData_BookShope.bookName, proj.StockData_BookShope.main_sanf, proj.StockData_BookShope.moalef, proj.StockData_BookShope.DarNashr _
    ...
    Private Sub T_button3_Click_1(sender As Object, e As EventArgs) Handles T_button3.Click
        mydatac.Load(proj.dataConPath_MyData, StockBookShope.StockData_TabaleName, proj.StockData_BookShope.StockDataBookShope_Fm, SaleDGV, 0)
        mydatac.showDatabyReaderTocombobox(MainSanfCmp, "main_sanf", "StockDataBookShops")
        mydatac.showDatabyReaderTocombobox(Moalefcmp, "moalef", "StockDataBookShops")
        mydatac.showDatabyReaderTocombobox(DarNashrCmb, "DarNashr", "StockDataBookShops")
    ...
    Private Sub Rename()
        mydatac.MoveFirst()
        Dim mains, darnasrs, booknames, moalef As String
        Dim maini, darnasri, booknamei As Integer
        For i = 0 To mydatac.RowCount
            mains = SaleDGV.CurrentRow.Cells(3).Value
            darnasrs = SaleDGV.CurrentRow.Cells(5).Value
            booknames = SaleDGV.CurrentRow.Cells(2).Value
            moalef = SaleDGV.CurrentRow.Cells(4).Value
            SaleDGV.CurrentRow.Cells.Item(1).Value = mains & " - " & booknames & "/تاليف " & moalef & " /طابعة " & darnasrs
            mydatac.MoveNext()
